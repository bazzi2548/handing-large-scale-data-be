...
public class LargeScaleService {
	...
	/**
	 * 일반적으로 ThreadPool을 Bean으로 선언해서 사용하는 편인데, (요청이 들어올 때 마다 스레드 풀이 과도하게 생성되는 것을 방지하기 위함)
	 * 학습 목적이므로 여기서는 그런 과정을 수행하지 않습니다.
	 * 혹시나 CompletableFuture에 대한 이해가 있다면, 이 방식을... 사용할... 수도...
	 */
	 ...
	 * @param size
	 */
	public PageInfo<Transaction> getTransactions(String accountNumber, String pageToken, TransactionSearchOption option,
		int size
	) {
		if (pageToken == null) {
	...
	public void validateAccountNumber(int pageSize) {
		// 다 들고오는건 무리 -> 쪼개서 들고오자
		// 1000건 단위로 들고온다고 치면... 2천만개 -> 2만번 쿼리 때려서 부정합 데이터를 확인해주면 되겠다.
		// 개발중에, 테스트 한다고 2만번을 다 때리면 너무 오랜시간이 걸림
		// 개발 테스트용으로, 초반 몇 페이지만 조회하고 말도록 만들것.
	...
	private void migrateUserInfo(User user) {
		...
		C4QueryExecuteTemplate.<Account>selectAndExecuteWithCursor(LIMIT_SIZE,
			lastAccount -> accountRepository.findAccountByUserIdAndLastAccountId(user.getId(),
				lastAccount == null ? null : lastAccount.getId(), LIMIT_SIZE),
			accounts -> {
				...
				accounts.forEach(account -> transactionRepository.selectAndMigrate(account,
					C4StringUtil.format("transaction_migration_jeonghwan_{}", groupId)));
			});
	}
}
