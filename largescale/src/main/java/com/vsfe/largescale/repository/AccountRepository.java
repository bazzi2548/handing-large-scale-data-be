...
public class AccountRepository {
	...
	/**
	 * 주어진 lastAccountId를 활용하여, size 만큼의 account 정보를 가져온다.
	 * @param lastAccountId
	 * @param size
	 * @return
	 */
	...
	public void saveAll(int groupId, List<Account> accounts) {
		var sql = C4StringUtil.format("""
			INSERT INTO account_migration_jeonghwan_{} (account_id, account_number, user_id, account_type, memo, balance, create_date, recent_transaction_date)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?)
			""", groupId);
		...
		jdbcTemplate.batchUpdate(sql,
			...
			(PreparedStatement ps, Account account) -> {
				...
				ps.setObject(7, account.getRecentTransactionDate());
